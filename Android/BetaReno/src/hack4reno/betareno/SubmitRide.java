...
{
	protected SubmitRide submitRide;
    ...
    public void onCreate(Bundle savedInstanceState) 
    {
        ...
        datePlan.setVisibility(View.INVISIBLE);
        timePlan.setVisibility(View.INVISIBLE);
		...
		chkPlan.setOnCheckedChangeListener(new OnCheckedChangeListener()
		{
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) 
			{
				if (isChecked)
				{
			        datePlan.setVisibility(View.VISIBLE);
		...
		btnSubmit.setOnClickListener(new OnClickListener()
		{
			public void onClick(View v) 
			{
				...
				if (editWhat.getText().toString().equals(""))				
					HelperFunctions.say(submitRide, getString(R.string.submit_error_what));
				else if (editWhere.getText().toString().equals(""))				
					HelperFunctions.say(submitRide, getString(R.string.submit_error_where));			
				// Begin upload
				else
				{	
    ...
    private void setLocationAndUpload(String address)
    {
    	...
    	final Dialog dialogStartAddressSuggestions = new Dialog(submitRide);
    	...
    	inputDialog = new Dialog(submitRide);
		...
		try
		{
			...
			approximateLocationPoint = HelperFunctions.getLastKnownLocation(submitRide);
		}
    	...
    	lstStartSuggestions.setOnItemClickListener(new OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> a, View v, int pos, long id)
			{
				...
				idea.setLatitude(HelperFunctions.latitude1e6toString(exactLocationPoint));
				idea.setLongitude(HelperFunctions.longitude1e6toString(exactLocationPoint));
    ...
    // This function pulls possible locations from google maps
 	private class GetSuggestions extends AsyncTask<String, Integer, String>
 	{
 		...
 		{
 			pd = ProgressDialog.show(submitRide, "", "Finding Your Address...");
 			pd.show(); 			
 		}
 		...
 		protected void onPostExecute(String result)
 		{
 			...
 			{
 				new AlertDialog.Builder(submitRide).setMessage(
 			...
 			{
 				new AlertDialog.Builder(submitRide)
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent imageReturnedIntent)
	{
		...
		case REQ_CODE_PICK_IMAGE:
			if (resultCode == RESULT_OK)
			{
				...
				btnPic.setText(m_userSelectedImagePath);
			}
		}
	}
    ...
    private class UploadIdea extends AsyncTask<HttpResponse, Integer, Idea>
	{
		...
		protected void onPreExecute()
		{
			pd = new ProgressDialog(submitRide);
		...
		protected Idea doInBackground(HttpResponse... arg0)
		{
			...
			try
			{
				...
				return new Idea(jsonObject.getJSONObject("idea"));
			}
		...
		protected void onPostExecute(final Idea idea)
		{
			...
			{
				// TODO - launch the reader thing
				HelperFunctions.say(submitRide, idea.getWhat());
